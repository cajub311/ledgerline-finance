...
export function ImportPage({ state, onStateChange }: ImportPageProps) {
  ...
  const handleWizardCsvFile = async () => {
    ...
    try {
      ...
      const file = files.find((f) => {
        ...
        return n.endsWith('.csv') || n.endsWith('.tsv') || f.type === 'text/tab-separated-values';
      });
      if (!file) {
  ...
  const exportXlsxFull = async () => {
    ...
    try {
      setBusy(true);
      const { financeBackupToXlsxArrayBuffer } = await import('../finance/export.xlsx');
      const buffer = financeBackupToXlsxArrayBuffer(state);
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `ledgerline-backup-${new Date().toISOString().slice(0, 10)}.xlsx`;
      link.click();
      URL.revokeObjectURL(url);
      notify('Exported full backup as Excel (multi-sheet).', 'success');
    } catch (error) {
    ...
    <View style={{ gap: spacing.lg }}>
      <View>
        ...
        <Text style={[styles.subtitle, { color: palette.textMuted }]}>
          Upload bank exports (CSV, TSV, XLSX, PDF), map columns when headers do not match, paste text, or export a
          full backup.
        </Text>
      </View>
      ...
      <Card title="CSV / TSV import wizard" eyebrow="Preview · map columns · dedupe">
        <Text style={{ color: palette.textMuted, fontSize: typography.small, lineHeight: 19, marginBottom: spacing.md }}>
          For tab- or comma-separated exports: we auto-suggest column roles. Preview the first 10 rows, adjust
      ...
      <View style={styles.grid}>
        <Card title="Upload files" eyebrow="CSV · TSV · XLSX · PDF" style={styles.flex1}>
          <Text style={{ color: palette.textMuted, fontSize: typography.small, lineHeight: 19 }}>
            We detect delimiter and column headers for most banks. PDFs use on-device text extraction; scanned
            statements may need CSV export from your bank. Duplicates are skipped by date, payee, and amount.
          </Text>
          <Button
      ...
      <Card title="Backups" eyebrow="Full state">
        ...
        <View style={{ flexDirection: 'row', gap: spacing.sm, flexWrap: 'wrap' }}>
          ...
          <Button label="Export Excel backup" variant="secondary" onPress={exportXlsxFull} disabled={busy} />
