...
- **2026‑04‑17 — T‑04 (upcoming calendar):** `projectRecurring`, Dashboard "Upcoming in the next 30 days", Forecast month calendar (`cursor/upcoming-calendar-73ac`).
- **2026‑04‑17 — Wells Fargo import fix:** CSVs with slash‑style headers (`Deposits/Additions`, `Withdrawals/Subtractions`) and other bank wordings (`Post Date`, `Transaction`, `Amount (USD)`) now import correctly. Added `fixtures/statements/wells-fargo-checking-2024.csv` and 3 new tests covering 100% row capture. (`cursor/wells-fargo-import-fix-f766`).
- **2026‑04‑17 — Opus — T‑05 (tags):** `FinanceTransaction.tags`, `normalizeTag` / `normalizeTags`, bulk `setTransactionsTags` (merge / replace / remove), `getAllTags`, CSV + QIF export columns, rule `addTags` + `markReviewed`, tag chips on transaction rows with one‑click drill‑down, tag filter dropdown, bulk add/remove tag inputs in the bulk bar, tag editor with suggestion chips in the edit modal.
- **2026‑04‑17 — Opus — T‑08 (command palette):** `useHotkeys` (web‑only), `CommandPalette` overlay mounted in FinanceApp with ⌘K / Ctrl+K toggle, arrow/enter navigation, fuzzy search over label/hint/keywords, grouped by section. Ships actions for every nav tab, theme toggle, CSV export, PDF export, JSON backup. Sidebar shows a ⌘K discoverability chip.
- **2026‑04‑17 — Opus — a11y polish:** Button gains `accessibilityLabel` / `accessibilityHint` / `accessibilityState.disabled`. Transaction rows announce payee + amount + date + account + category + review state to screen readers.
